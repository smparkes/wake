...
1. Perhaps add the event type to the watch callback, since multiple events can fire now. Could be useful, for example, deleting HTML files generated from Haml files when the Haml files are deleted. Should check to make the file hasn't been manually modified via a hash.
...
1. Only blitz HTML files with Haml conversion if they haven't changed, probably via a hash.

1. Don't run html/js paired specs twice on load